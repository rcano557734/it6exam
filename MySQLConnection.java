...
public class MySQLConnection {
    ...
    private void createDatabaseAndTables(Connection con) {
        try {
            ...
            // FIXED FOREIGN KEYS TO MATCH THE PROPER TABLES
            String createReservationTable = "CREATE TABLE IF NOT EXISTS reservation_tbl ("
                    + "reservation_id INT AUTO_INCREMENT PRIMARY KEY, "
                    + "guest_id INT, "
                    + "room_id INT, "
                    + "date VARCHAR(50), "
                    + "FOREIGN KEY (guest_id) REFERENCES guest_tbl(guest_id) ON DELETE CASCADE ON UPDATE CASCADE, "
                    + "FOREIGN KEY (room_id) REFERENCES room_tbl(room_id) ON DELETE CASCADE ON UPDATE CASCADE"
                    + ")";
            st.executeUpdate(createReservationTable);

            System.out.println("Hotel Database and tables checked/created successfully!");
